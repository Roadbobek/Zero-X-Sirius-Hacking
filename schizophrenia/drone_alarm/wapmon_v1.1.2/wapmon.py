# WAP Monitor - wapmon
# By Roadbobek, v1.1.2 (Another Linux nmcli parsing fix)
...
def scan_waps():
    ...
    if sys.platform == "win32":
        ...
        if sys.platform == "darwin":
            ...
            # 1. Execute rescan command (remains the same)
            try:
                subprocess.run(
            ...
            # 2. List the results in terse mode (remains the same)
            ...
            try:
                ...
                # FIX: Use string manipulation based on fixed BSSID length (22 chars escaped)
                BSSID_ESCAPED_LENGTH = 22

                for line in lines:
                    try:
                        # 1. Split from the end to reliably isolate SIGNAL and CHAN
                        # This works regardless of the number of colons in BSSID:SSID
                        signal_chan_str = line.rsplit(':', 2)[-2:]
                        signal_percent = signal_chan_str[0].strip()
                        channel = signal_chan_str[1].strip()

                        # The remaining string is BSSID:SSID (escaped colons still present)
                        bssid_ssid_raw = line.rsplit(':', 2)[0]
                    except IndexError:
                        continue  # Skip malformed lines

                    # 2. Extract BSSID and SSID using fixed length slicing

                    # Ensure the string is long enough for an escaped BSSID and at least a separator
                    if len(bssid_ssid_raw) < BSSID_ESCAPED_LENGTH:
                        continue

                    # BSSID is exactly the first 22 characters (XX\:XX\:XX\:XX\:XX\:XX)
                    bssid_raw_escaped = bssid_ssid_raw[:BSSID_ESCAPED_LENGTH].strip()

                    # SSID starts one character after the BSSID (the unescaped ':' separating BSSID and SSID)
                    # NOTE: We skip the 22nd character (which should be the ':')
                    ssid_raw_escaped = bssid_ssid_raw[BSSID_ESCAPED_LENGTH + 1:].strip()

                    # 3. Unescape and format the BSSID: CRITICAL STEP
                    # Turns '38\:06\:...' into '38:06:...'
                    bssid_formatted = bssid_raw_escaped.replace('\:', ':').upper()

                    # 4. Unescape and clean the SSID
                    ssid = ssid_raw_escaped.replace('\:', ':')
