...
def display_initial_waps(waps_dict):
    """
    Prints all WAPs currently in the cache upon initialization, using logging 
    so that details are recorded in the log file (LOG-INFO) but suppressed 
    ...
    if count == 0:
        log_scan_info(separator)
        log_scan_info("--- Initial WAP Cache Content: No WAPs detected. ---")
        log_scan_info(separator)
        return

    # Log header to file (LOG-INFO) 
    log_scan_info(f"\n{separator}")
    log_scan_info(f"--- Initial WAP Cache Content ({count} WAP{'s' if count > 1 else ''} Detected) ---")
    
    # Sort for consistent display (e.g., by SSID then BSSID) 
    sorted_waps = sorted(waps_dict.values(), key=lambda w: (w['ssid'] or "ZZZ", w['bssid']))
    
    # Log header for alignment (LOG-INFO)
    log_scan_info(f"{'BSSID':<20} {'SSID':<30} {'Signal':<10} {'Channel':<10} {'Band':<10} {'Auth':<15}")
    log_scan_info(separator)
    ...
    for wap in sorted_waps:
        ...
        # Log the detailed line (LOG-INFO)
        log_scan_info(f"{bssid:<20} {ssid_display:<30} {signal:<10} {channel:<10} {band:<10} {auth:<15}")
...
def monitor_loop():
    ...
    try:
        ...
        # Display/Log the initial WAP details (uses log_scan_info - level 15)
        ...
        while True:
            ...
            if new_bssids or removed_bssids:
                ...
                # FIX: If no changes, use log_scan_info (level 15) to ensure it is 
...
def setup_logging(log_enabled):
    ...
    # FIX: Set root logger level to SCAN_LOG_LEVEL (15) so it doesn't filter out 
