...
class AlertFormatter(logging.Formatter):
    ...
    'ALERT' string in the final output, while maintaining the correct log level 
    for filtering.
    """
...
def scan_waps():
    ...
    if sys.platform == "win32":
        ...
        # ... (scanning logic for macOS/Linux remains the same)
...
# ... (display_initial_waps remains the same)
    ...
    try:
        ...
        while True:
            ...
            if new_bssids or removed_bssids:
                ...
                # User requirement: If no changes, ONLY log this status message to the file (logging.INFO is suppressed from CLI).
...
def setup_logging(log_enabled):
    ...
    ALERT_FORMAT = '[%(asctime)s] %(levelname)s: %(message)s' # Time ALERT: Message (no extra '-')

    # 1. Console Handler (ALWAYS active for user feedback on errors/warnings/alerts)
    console_handler = logging.StreamHandler(sys.stdout)
    # Only show WARNING (now ALERT) and above for system/scan errors and WAP alerts.
    ...
    if log_enabled:
        ...
        try:
            ...
            file_handler.setLevel(logging.INFO) # Capture all INFO and above for full history
            
            # Use the custom formatter for ALERT mapping and the detailed file format
            file_handler.setFormatter(AlertFormatter('%(asctime)s - %(levelname)s - %(message)s'))
            root_logger.addHandler(file_handler)
            print(f"[{time.strftime('%H:%M:%S')}] File logging enabled. Event history saved to: {LOG_FILE}")
        except Exception as e:
