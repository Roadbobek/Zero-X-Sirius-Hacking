...
class AlertFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        # USER REQUEST: Display 'LOG-INFO' for custom level 15 messages in the log file
...
def log_scan_info(message):
    """Helper function to log scan details using the custom SCAN_LOG_LEVEL."""
    ...
    if sys.platform == "win32":
        ...
        for attempt in range(1, MAX_LSWIFI_RETRIES + 1):
            try:
                ...
                if attempt == 1:
                    # USER REQUEST: Use log_scan_info to hide from console, but log to file as LOG-INFO
        ...
        if sys.platform == "darwin":
            ...
            try:
                ...
                logging.critical(f"Linux 'nmcli' failed. Cannot retrieve WAPs. ({type(e).__name__})")
...
def display_initial_waps(waps_dict):
    ...
    print("\n--------------------------------------------------------------------------------------------------")
    # Log to file, but print to console without timestamp to avoid double timestamping if logging.info() was used here
    # The monitor_loop starting message already covers the timestamping for the main process flow.
...
def monitor_loop():
    ...
    try:
        ...
        # This line is printed *after* the initial scan completes
        ...
        while True:
            ...
            if new_bssids or removed_bssids:
                ...
                # User requirement: If no changes, ONLY log this status message to the file as standard INFO.
                logging.info(f"Scan complete: No new or removed WAPs detected.")
...
def setup_logging(log_enabled):
    ...
    # Set level to INFO (20). Since SCAN_LOG_LEVEL is 15, it will be ignored on the console.
    ...
    if log_enabled:
        ...
        try:
            # USER REQUEST: Insert an empty line at the start of a new session in the log file.
...
def main():
    ...
    if ALARM_ENABLED:
        ...
        # FIX: Use logging.info() for timestamping and logging.
        logging.info("Alarms enabled (--alarm/-a).")
    else:
        # If alarms are not enabled, we can optionally log that status too.
...
if __name__ == "__main__":
    ...
    # It's cleaner to define it globally if used by a helper function outside main.
    main()
