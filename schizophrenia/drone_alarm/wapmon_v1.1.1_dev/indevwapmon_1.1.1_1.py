...
def scan_waps():
    ...
    if sys.platform == "win32":
        ...
        if sys.platform == "darwin":
            ...
            # Requested fields: BSSID, SSID, SIGNAL, CHAN
            ...
            try:
                ...
                # FIX: Implement robust parsing for nmcli -t output to handle escaped colons (\:) in SSIDs
                for line in lines:
                    ...
                    # We expect at least 4 parts (BSSID, SSID, SIGNAL, CHAN).
                    if len(parts) < 4:
                        continue

                    # BSSID is always the first part (index 0)
                    bssid = parts[0]
                    
                    # SIGNAL and CHAN are always the last two parts (indices -2 and -1)
                    signal_percent = parts[-2]
                    channel = parts[-1]
                    
                    # SSID is everything in between (from index 1 up to index -2)
                    # We re-join the middle parts to reconstruct the raw SSID string
                    ssid_raw = ':'.join(parts[1:-2])
                    
                    # FIX: Unescape the SSID by replacing '\:' (used by nmcli) with a simple ':'
                    # This removes the unwanted backslash and correctly reconstructs the SSID.
                    ssid = ssid_raw.replace(r'\:', ':') 
                    
                    if not bssid or not signal_percent or not channel:
