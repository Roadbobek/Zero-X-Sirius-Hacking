...
# -f "udp port X and data": Captures only UDP traffic on the port AND only if it has a payload
# -T fields: Sets the output format to fields
# -e data: Extracts the raw data payload as hexadecimal string (the fix for video data)
# -l: Flush output immediately (essential for real-time streaming)
TSHARK_CMD_TEMPLATE = 'sudo tshark -i {} -f "udp port {} and data" -T fields -e data -l'
MAX_RETRIES = 5
...
def stream_payload_data(interface, port):
    ...
    print(f"**TShark field corrected from 'data.text' to 'data'.**")
    ...
    for attempt in range(MAX_RETRIES):
        try:
            # Check for sudo permission explicitly before starting the loop
            ...
            while True:
                ...
                if line:
                    ...
                    # Only print if the line contains actual data (not just an empty string)
                    if stripped_line:
                        print(stripped_line)
