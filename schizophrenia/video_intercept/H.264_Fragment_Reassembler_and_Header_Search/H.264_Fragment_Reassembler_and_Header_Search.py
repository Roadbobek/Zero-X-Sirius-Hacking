...
# The protocol is likely stripping the Start Code. We look for the raw NAL Type byte instead.
    ...
    try:
        ...
        with open(filepath, mode='r', encoding='latin-1') as file:
            ...
            for i, row in enumerate(reader):
                # We assume the raw hex is in column 7 (the 8th column) based on the input structure
                if len(row) < 8:
                    continue

                raw_data_field = row[7].strip()
                
                # Use regex to find and capture the actual hex sequence, stripping prefix characters like [â\200\246]
...
def extract_fragment_info(raw_hex_packet):
    ...
    try:
        ...
        # Skipping malformed/short packet: {e}
...
def search_for_headers(reassembled_frames):
    """
    Searches the reassembled frames for the H.264 SPS and PPS NAL unit type bytes.
    If found, it manually prepends the missing Start Code Prefix (00 00 00 01).
    """
    found_sps = None
    found_pps = None
    ...
    for frame_id, frame_data in reassembled_frames.items():
        ...
        if sps_index != -1 and not found_sps:
            # Found the start of the SPS NAL unit (the 0x67 byte).
            
            # Look for the next Start Code Prefix (00 00 00 01) to define the end of the current NAL unit
            next_scp_index = frame_data.find(NAL_START_CODE, sps_index + 1)
            
            # The raw NAL data starts at sps_index and ends at the next SCP, or the end of the frame
            raw_nal_data = frame_data[sps_index : next_scp_index] if next_scp_index != -1 else frame_data[sps_index:]
            
            # Manually prepend the H.264 Start Code Prefix (00 00 00 01)
            full_sps_nal_unit = NAL_START_CODE + raw_nal_data
            
            found_sps = {
                'frame_id': frame_id, 
                'data': full_sps_nal_unit
            }
        ...
        if pps_index != -1 and not found_pps:
            # Found the start of the PPS NAL unit (the 0x68 byte).
...
if __name__ == "__main__":
    ...
    if sps_result and pps_result:
        ...
        print("ACTION: Ensure the new search logic didn't break anything, or that the video data is present.")
