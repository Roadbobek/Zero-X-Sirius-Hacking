...
def load_packets_from_csv(filepath):
    ...
    try:
        # Use latin-1 encoding which is safer for raw binary data exported as hex
...
def search_for_headers(reassembled_frames):
    ...
    # 0x65 is the NAL unit type for a coded slice of an IDR picture (Fragment index 0 is sometimes 0x65)
    ...
    if idr_slice_index == -1:
        # Fallback: Assume the PPS ends at 8 bytes and everything after is IDR.
        # This is a very aggressive assumption but necessary if the boundary isn't clear.
...
def write_h264_stream(reassembled_frames, sps_result, pps_result, idr_frame_id, idr_slice_data):
    """
    Writes two H.264 streams: 
    1. output_full.h264 (with headers)
    2. output_no_headers.h264 (without headers, for FFmpeg bypass)
    """
    ...
    try:
        # --- File 1: Full Stream (output_full.h264) ---
        with open(output_full_filename, 'wb') as f:
            # 1. Write the CLEANED SPS header
            f.write(NAL_START_CODE)
            f.write(cleaned_sps)
            
            # 2. Write the CLEANED PPS header
            f.write(NAL_START_CODE)
            f.write(cleaned_pps)
            
            # 3. Write the initial CLEANED IDR Slice
            f.write(NAL_START_CODE)
            f.write(cleaned_idr)
            total_nal_units = 3

            # 4. Write all subsequent frames
        ...
        print(f"Wrote FULL stream (SPS, PPS, 98 slices) to {output_full_filename}.")
        
        # --- File 2: Headerless Stream (output_no_headers.h264) ---
        nal_units_no_headers = 0
        with open(output_no_headers_filename, 'wb') as f:
            # 1. Start directly with the initial CLEANED IDR Slice
            f.write(NAL_START_CODE)
            f.write(cleaned_idr)
            nal_units_no_headers += 1

            # 2. Write all subsequent frames (same as above)
            for frame_id, frame_data in reassembled_frames.items():
                if frame_id == idr_frame_id:
                    continue
        ...
        print(f"✅ FINAL SUCCESS: Wrote a total of {total_nal_units} NAL units across both files.")
        print("\n>>> NEXT STEP: Attempt to play the headerless file using the forced FFmpeg command!")
...
if __name__ == "__main__":
    ...
    if sps_result and pps_result and idr_slice_data:
        ...
        # Phase 5: Write the H.264 file (Applies EPB removal and writes two versions)
