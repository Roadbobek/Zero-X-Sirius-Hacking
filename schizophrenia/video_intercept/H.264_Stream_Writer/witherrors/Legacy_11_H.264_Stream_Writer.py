...
def search_for_headers(reassembled_frames):
    """
    Searches for original headers, segments payloads, and handles PPS truncation/repair.
    We still need to find the IDR frame to know where the video stream starts.
    """
    found_sps = None
    found_pps = None
    idr_frame_id = None
    ...
    # We only care about frame 0100 for the IDR slice data
    ...
    # Search for the next NAL slice type byte (0x65, 0x41, etc.)
    idr_slice_index = -1
    ...
    # Return the IDR slice data which contains the proprietary header
...
def write_final_clean_stream(reassembled_frames, idr_frame_id, idr_slice_data):
    ...
    try:
        with open(output_final_filename, 'wb') as f:
            ...
            # CRITICAL STEP: STRIP THE FIRST 19 BYTES OF PROPRIETARY METADATA
            final_idr = cleaned_idr[PROPRIETARY_HEADER_SKIP_BYTES:]
            ...
            for frame_id, frame_data in reassembled_frames.items():
                ...
                # CRITICAL STEP: STRIP THE FIRST 19 BYTES OF PROPRIETARY METADATA
                final_frame = cleaned_frame[PROPRIETARY_HEADER_SKIP_BYTES:]
        ...
        if os.path.exists(output_final_filename):
            ...
            with open(output_final_filename, 'rb') as f:
                # Skip the injected headers (SPS is 34 bytes, PPS is 4 bytes + 2 start codes)
                data = f.read(100)
