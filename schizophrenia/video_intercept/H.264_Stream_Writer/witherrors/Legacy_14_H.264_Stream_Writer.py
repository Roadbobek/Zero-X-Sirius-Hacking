...
# NOTE: If you need 1920x1080 (as mentioned in your prompt), replace these 
# with the correct 1920x1080 SPS/PPS bytes! 
# The current values will result in 1280x720 video output.
...
def search_for_headers(reassembled_frames):
    ...
    NOTE: This logic is used primarily to find the boundary of the IDR slice.
    The actual SPS/PPS injected later are the hardcoded ones above.
    """
...
def write_final_clean_stream(reassembled_frames, idr_frame_id, idr_slice_data):
    ...
    try:
        with open(output_final_filename, 'wb') as f:
            ...
            # CRITICAL FIX: Ensure every NAL unit starts with the NAL_START_CODE
        ...
        if os.path.exists(output_final_filename):
            ...
            # Check the start of the file to confirm the first NAL unit is correctly prefixed
            first_bytes = data[:4]
            if first_bytes == NAL_START_CODE:
                print("SUCCESS CONFIRMED: File correctly starts with NAL Start Code (00 00 00 01).")
            
            # Find the third NAL start code (the start of the video data)
            # Search for the start of the IDR slice (the 3rd NAL unit)
            # Find the 1st NAL start code (SPS)
            sps_start = data.find(NAL_START_CODE)
            # Find the 2nd NAL start code (PPS)
            pps_start = data.find(NAL_START_CODE, sps_start + 4)
            # Find the 3rd NAL start code (IDR Slice)
            ...
            if idr_start != -1:
                ...
                print(formatted_hex)
                # An IDR slice NAL unit header typically starts with 0x05 or 0x65
                if diagnostic_hex.startswith('05') or diagnostic_hex.startswith('65'):
                    print("SUCCESS CONFIRMED: The slice payload now correctly starts with a slice header.")
                else:
                    print("WARNING: The stream does not start with a standard IDR slice header ('05' or '65').")
            else:
                print("Could not find the third NAL start code in the cleaned file for diagnostic check.")
...
if __name__ == "__main__":
    ...
    # We pass the full set of reassembled frames, but search for headers specifically in '0100'
    sps_found, pps_found, idr_frame_id, idr_slice_data = search_for_headers(reassembled_frames)
