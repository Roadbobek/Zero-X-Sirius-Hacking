...
def extract_fragment_info(raw_hex_packet):
    ...
    try:
        ...
        # Fragment index is an integer for ordering
...
def search_for_headers(reassembled_frames):
    ...
    It returns raw payloads without NAL_START_CODE prepended.
    """
    found_sps = None
    found_pps = None
    idr_frame_id = None
    ...
    print("\n--- Phase 3: Searching for SPS/PPS Headers (Refined Segmentation) ---")
    
    # Check the keyframe (0100) first as that's where the keyframe was found
    for frame_id, frame_data in reassembled_frames.items():
        if frame_id != '0100':
            continue

        # 1. Search for SPS NAL Type byte (0x67) - SPS is expected near the start
        sps_index = frame_data.find(SPS_NAL_TYPE)
        if sps_index != -1 and not found_sps:
            
            # 2. Search for PPS NAL Type byte (0x68)
            # Scan in a small window (up to 100 bytes) after the SPS starts to account for padding
            pps_index = -1
            for i in range(sps_index + 1, min(sps_index + 100, len(frame_data))):
            ...
            if pps_index != -1 and not found_pps:
                ...
                # 3. Search for the next NAL slice type byte (0x65 for IDR slice, 0x41 for P-slice)
                idr_slice_index = -1
                
                # Search for the next NAL type in a window (e.g., 50 bytes after PPS starts)
                for nal_type in [b'\x65', b'\x41', b'\x05', b'\x01']: 
                    # Search window starts 1 byte after PPS type byte
                    idr_slice_index_raw = frame_data.find(nal_type, pps_index + len(PPS_NAL_TYPE))
                    
                    if idr_slice_index_raw != -1:
                         # Use the earliest successful index found
                         if idr_slice_index == -1 or idr_slice_index_raw < idr_slice_index:
                             idr_slice_index = idr_slice_index_raw

                if idr_slice_index != -1:
                    # Segment 2: PPS NALU Payload (from 0x68 up to, but not including, the IDR slice start)
