...
def search_for_headers(reassembled_frames: Dict[str, bytes]) -> Tuple[bool, bool, Optional[str], Optional[bytes], Optional[bytes], Optional[bytes]]:
    ...
    for i in range(search_start, search_end_slice):
        ...
        # We look for a byte that has a NAL unit type != 7 (SPS) and != 8 (PPS)
...
def write_final_clean_stream(
    ...
    try:
        with open(output_final_filename, 'wb') as f:
            ...
            if len(idr_slice_data) == 0:
                print("Warning: IDR slice data is empty. Skipping writing the IDR slice.")
                final_idr_slice = b''
            else:
                # --- NAL Unit Type Fix for Keyframe IDR Slice ---
                # The NAL unit type must be 5 (IDR slice) for the first frame.
                # The NAL Unit Header is the first byte of the slice payload (before Exp-Golomb data).
                first_nal_byte_int = idr_slice_data[0]
                nal_unit_type = first_nal_byte_int & 0x1F # Mask to get NAL Type (last 5 bits)

                if nal_unit_type != 5:
                    # Force NAL Unit Type 5 (IDR) with nal_ref_idc=3 (0x65)
                    # 0x65 is binary 01100101 -> F=0, NRI=3 (High reference), Type=5 (IDR Slice)
                    print(f"--> WARNING: IDR slice NAL header byte detected as Type {nal_unit_type}. Forcing to 0x65 (IDR NAL Unit Type 5).")
                    
                    # Replace the first byte with 0x65 (IDR NAL unit header)
                    fixed_idr_slice = b'\x65' + idr_slice_data[1:]
                else:
                    # If it's already type 5, use the original data
                    fixed_idr_slice = idr_slice_data
        ...
        if os.path.exists(output_final_filename):
            ...
            if len(start_codes) >= 3:
                ...
                if nal_unit_type == 5:
                    print("SUCCESS CONFIRMED: IDR Slice NAL unit type 5 found at start of payload after fix.")
                else:
                    print("WARNING: NAL Type is not 5, the IDR fix failed or the slice is corrupt.")
            else:
