...
# Define a safe maximum length for PPS. If the segmented PPS is longer, we assume 
# the excess bytes belong to the IDR slice and must be truncated and re-attached.
...
def search_for_headers(reassembled_frames):
    ...
    If PPS is too long (based on MAX_SAFE_PPS_LENGTH), it truncates the PPS and 
    prepends the excess bytes to the IDR slice.
    """
    found_sps = None
    found_pps = None
    idr_frame_id = None
    ...
    print("\n--- Phase 3: Searching for SPS/PPS Headers (Truncation Repair) ---")
    ...
    search_end_slice = min(search_start + 50, len(frame_data)) # Broader search for the next NAL type

    # Check for common slice NAL types (IDR 0x65, P-Slice 0x41/0x61, 0x05/0x01)
