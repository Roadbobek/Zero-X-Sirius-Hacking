...
def search_for_headers(reassembled_frames):
    """
    Searches for the SPS/PPS NAL unit type bytes and segments the payloads.
    This version returns the full segmented payloads for EPB removal in the next step.
    """
    found_sps = None
    found_pps = None
    idr_frame_id = None
    ...
    print("\n--- Phase 3: Searching for SPS/PPS Headers (Boundary Detection) ---")
    ...
    # Search for PPS in a window (up to 100 bytes) after SPS starts
    pps_index = -1
    ...
    # Search for the next NAL type in a window (e.g., 50 bytes after PPS starts)
    idr_slice_index = -1
    ...
    # Note: 0x05 is IDR slice, 0x01 is non-IDR slice. 0x65 is the FUA/STAPA equivalent.
    ...
    print(f"--> FOUND SPS in Frame ID {frame_id}! (Length: {len(raw_sps_payload)} bytes)")
    print(f"--> FOUND PPS in Frame ID {frame_id}! (Length: {len(raw_pps_payload)} bytes)")
...
def write_h264_stream(reassembled_frames, sps_result, pps_result, idr_frame_id, idr_slice_data, output_filename="output.h264"):
    ...
    ensuring all NAL unit payloads are cleaned of Emulation Prevention Bytes.
    """
    print(f"\n--- Phase 5: Writing H.264 Stream to {output_filename} (EPB Removal Applied) ---")
    ...
    try:
        with open(output_filename, 'wb') as f:
            ...
            print(f"Wrote CLEANED SPS ({len(cleaned_sps)} bytes) and PPS ({len(cleaned_pps)} bytes) Headers.")
            ...
            print(f"✅ FINAL SUCCESS: Wrote a total of {frames_written} CLEANED NAL units to {output_filename}.")
